...
feature 'Caching' do
  ...
  before { page.driver.header 'Content-status', 'application/json' }
  ...
  describe 'GET /statuses/:id' do
    ...
    let(:cache_key) { ActiveSupport::Cache.expand_cache_key(['status_serializer', resource.cache_key, 'to-json']) }
      ...
      describe 'when sends the If-Modified-Since header' do
        ...
        describe 'when the resource changes' do
          ...
          it 'creates a new fragment cache' do
            new_key = ActiveSupport::Cache.expand_cache_key(['status_serializer', resource.cache_key, 'to-json'])
  ...
  #describe 'GET /statuses' do
    ...
    #let!(:resource) { FactoryGirl.create :status, resource_owner_id: user.id }
    #let(:uri)       { "/statuses/#{resource.id}" }

    #let(:cache_json_key) { ActiveSupport::Cache.expand_cache_key(['status_serializer', resource.cache_key, 'to-json']) }
    #let(:cache_hash_key) { ActiveSupport::Cache.expand_cache_key(['status_serializer', resource.cache_key, 'serializable-hash']) }
